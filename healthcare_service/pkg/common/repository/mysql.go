...
func (cr CommonRepo) GetDoctorsList(ctx context.Context, address entity.Address) ([]entity.Doctor, error) {
	...
	sqlQuery := "SELECT name, phone, ST_X(location), ST_Y(location), appointment_fees, FROM doctors WHERE ST_Distance_Sphere(location, POINT(?, ?)) <= 10000 and status = ?"
	...
	for rows.Next() {
		...
		err = rows.Scan(&doctor.Name, &doctor.Phone, &doctor.Address.Latitude, &doctor.Address.Longitude, &doctor.Fees)
		if err != nil {
